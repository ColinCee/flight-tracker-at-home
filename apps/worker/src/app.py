"""Flight Tracker at Home API — Cloudflare Worker edition.

Uses a create_app() factory so the Worker entry point can inject
environment variables before the FastAPI app reads them.
"""
...
def create_app() -> FastAPI:
    ...
    async def get_aircraft() -> AircraftResponse:
        """Main polling endpoint. Returns cached airspace state with KPIs."""
        state = await airspace_cache.get_state()
        return state

    @app.get(
        "/debug/opensky",
        operation_id="debugOpensky",

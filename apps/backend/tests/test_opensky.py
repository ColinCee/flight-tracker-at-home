import math
import pytest
from src.opensky import parse_state_vector, calculate_distance_km, check_lhr_approach
...
def test_parse_state_vector_valid():
  ...
  raw_vector = [
    "400a5b", "BAW123  ", "United Kingdom", 1690000000, 1690000000,
    -0.45, 51.47, 1000.0, False, 150.0,
    90.0, -5.0, None, 1050.0, "7700",
    False, 0
  ]
...
def test_parse_state_vector_missing_coords():
  ...
  raw_vector = [
    "400a5b", "BAW123", "United Kingdom", 1690000000, 1690000000,
    None, None, 1000.0, False, 150.0,  # null lon/lat
    90.0, -5.0, None, 1050.0, "7700",
    False, 0
  ]
...
def test_check_lhr_approach_valid():
  ...
  aircraft = AircraftState(
    icao24="123456", callsign="BAW1", origin_country="UK", last_contact=0,
    latitude=51.4700, longitude=-0.5000,  # Very close to LHR
    baro_altitude=800.0,  # Below 1200m
    on_ground=False, velocity=100.0,
    true_track=92.0,  # Runway 09 heading
    vertical_rate=-3.0,  # Descending
    geo_altitude=850.0, squawk=None, is_approaching_lhr=False
  )
...
def test_check_lhr_approach_wrong_heading():
  ...
  aircraft = AircraftState(
    icao24="123456", callsign="BAW1", origin_country="UK", last_contact=0,
    latitude=51.4700, longitude=-0.5000,
    baro_altitude=800.0, on_ground=False, velocity=100.0,
    true_track=360.0,  # Northbound (fails track check)
    vertical_rate=-3.0, geo_altitude=850.0, squawk=None, is_approaching_lhr=False
  )

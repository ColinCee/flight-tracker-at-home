""" This class handles the extraction of the data from the opensky API, using Pydantic to check types and
putting the data into a dictionary. It then runs a series of checks to determine if an aircraft is inbound to
Heathrow.
"""
...
def check_lhr_approach(aircraft: AircraftState) -> bool:
  ...
  dist = calculate_distance_km(aircraft.latitude, aircraft.longitude, LHR_LAT, LHR_LON)
  if dist > 20.0:
    return False

  # If it passed all filters, it's on approach!
  return True
